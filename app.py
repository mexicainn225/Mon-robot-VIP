...
def est_autorise(user_id):
    # Accès prioritaire pour l'administrateur
...
async def valider(update, context):
    if str(update.effective_user.id) == ADMIN_ID and context.args:
        ...
        await update.message.reply_text(f"ID {context.args[0]} validé ✅")
...
async def recevoir_id(update, context):
    ...
    await update.message.reply_text("Ton ID a été transmis à l'administrateur.")
